...
public class EmployeeDAO implements EmployeeDAO_interface {
	...
	private static final String INSERT_STMT = "INSERT INTO EMPLOYEE (empNo, empName, empBirth, empTel, empSex, empPos, empSalary, empArrDate, empOff, empID, empAdd, empPic, empPassword, empEmail) VALUES (emp_seq.NEXTVAL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	private static final String GET_ALL_STMT = "SELECT empNo, empName, empBirth, empTel, empSex, empPos, empSalary, empArrDate, empOff,empID, empAdd, empPic, empPassword, empEmail FROM EMPLOYEE order by empNo desc";
	private static final String GET_ONE_STMT = "SELECT empNo, empName, empBirth, empTel, empSex, empPos, empSalary, empArrDate, empOff,empID, empAdd, empPic, empPassword, empEmail FROM EMPLOYEE where empNo = ?";
	private static final String DELETE = "DELETE FROM EMPLOYEE where empNo = ?";
	private static final String UPDATE = "UPDATE EMPLOYEE set empName = ?, empBirth= ?, empTel= ?, empSex= ?, empPos= ?, empSalary= ?, empArrDate= ?, empOff= ?, empID= ?, empAdd= ?, empPic= ?, empPassword= ? , empEmail= ? where empNo = ?";
	private static final String GET_LAST_EMPNO = "SELECT empNo FROM employee ORDER BY empNo DESC";
	@Override
	public void insert(EmployeeVO employeeVO) {
		...
		try {
			...
			pstmt.setString(13, employeeVO.getEmpEmail());
	...
	public void update(EmployeeVO employeeVO) {
		...
		try {
			...
			pstmt.setString(12, employeeVO.getEmpPassword());
			pstmt.setString(13, employeeVO.getEmpEmail());
			pstmt.setInt(14, employeeVO.getEmpNo());
...
//			con = DriverManager.getConnection(url, userid, passwd);
			...
			con = ds.getConnection();
			pstmt = con.prepareStatement(GET_LAST_EMPNO);
			rs = pstmt.executeQuery();
			
			if (rs.next()) {
				// aptVO  Domain objects
				
				return Integer.parseInt( rs.getString("empNo") );
