...
public class GbServletFront extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if("getOne_For_Display".equals(action)){// 來自select_page.jsp的請求
			...
			try{
				...
				String url = "/front/gb/listOneGb.jsp";
		...
		if("listGb_Bylostno".equals(action)){
			...
			try{
				...
				if(list==null){                                 //list不會是null 不可用list==null 改用isEmpty或size或length
					errorMsgs.add("查無資料");
				}
				if(!errorMsgs.isEmpty()){
					RequestDispatcher failureView = req
				...
				String url = "/front/gb/listGb_Bylostno.jsp";
...
//			try{
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				req.setAttribute("gbVO", gbVO);         // 資料庫取出的lostVO物件,存入req
				String url = "/front/gb/update_gb_input.jsp";
		...
		if ("update".equals(action)) { // 來自update_lost_input.jsp的請求
				...
				try {
					...
					if (!errorMsgs.isEmpty()) {
						...
						RequestDispatcher failureView = req
								.getRequestDispatcher("/front/gb/update_gb_input.jsp");
						failureView.forward(req, res);
						return; //程式中斷
					}
...
//					LostService lostSvc = new LostService();
					...
					String url = "/front/gb/after_updateGb_input.jsp";
//					if(requestURL.equals("/front/gb/listAllLost.jsp")){
//						req.setAttribute("listOneLost", gbSvc.getOneGb(gbno)); // 資料庫取出的list物件,存入request
//						url = "/front/gb/listAllLost.jsp?whichPage=" + whichPage + "&gbno=" + gbno;
//					}else if(requestURL.equals("/front/lost/listLost_ByState.jsp")){
//						req.setAttribute("listLost_ByState", gbSvc.listLost_ByState(lostVO.getLoststate()));
//						url = requestURL + "?lostno=" + gbno;
//					}else if(requestURL.equals("/front/lost/listLost_ByMemno.jsp")){
//						req.setAttribute("listLost_ByMemno", gbSvc.listLost_ByMemno(lostVO.getMemno()));
//						url = requestURL + "?lostno=" + gbno;
//					}
...
//					String url = "/front/lost/listAllLost.jsp";
...
//				req.setAttribute("list", gbSvc.listGb_Bymemno(gbVO.getMemno()));
				...
				if(requestURL.equals("/backend/gb/listGb_Bymemno.jsp")){
					req.setAttribute("listOneLost", gbSvc.listGb_Bymemno(gbVO.getMemno()));
