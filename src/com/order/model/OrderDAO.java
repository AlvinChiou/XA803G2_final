...
public class OrderDAO implements OrderDAO_Interface {
	...
	private static final String GET_ALL_STMT = 
			"SELECT ordno, to_char(ordtime, 'yyyy-mm-dd hh24:mi:ss') AS ordtime, ordaddr, ordtel, to_char(ordgotime, 'yyyy-mm-dd hh24:mi:ss') AS ordgotime, to_char(ordarrtime, 'yyyy-mm-dd hh24:mi:ss') AS ordarrtime ,to_char(orddeltime, 'yyyy-mm-dd hh24:mi:ss') AS orddeltime ,ordstate, memno, empno FROM pro_order WHERE orddeltime IS NULL ORDER BY ordtime DESC";
	...
	private static final String GET_PRODITEM_ByOrdNo_STMT = "SELECT itemno, itemqty, itemmemo, ordno, prono, price FROM Prod_Item WHERE ordno = ?";
	private static final String UPDATESTATUS = "UPDATE pro_order SET ordstate = ? where ordno =?";
	private static final String GET_BY_MEMNO = "SELECT ordno, to_char(ordtime, 'yyyy-mm-dd hh24:mi:ss') AS ordtime, ordaddr, ordtel, to_char(ordgotime, 'yyyy-mm-dd hh24:mi:ss') AS ordgotime, to_char(ordarrtime, 'yyyy-mm-dd hh24:mi:ss') AS ordarrtime,to_char(orddeltime, 'yyyy-mm-dd hh24:mi:ss') AS orddeltime,ordstate, memno, empno FROM pro_order WHERE memno = ? ORDER BY ordno DESC";
	private static final String GET_ORDERS_GOTIME_TODAY = "SELECT ordno, to_char(ordtime, 'yyyy-mm-dd hh24:mi:ss') AS ordtime, ordaddr, ordtel, to_char(ordgotime, 'yyyy-mm-dd hh24:mi:ss') AS ordgotime, to_char(ordarrtime, 'yyyy-mm-dd hh24:mi:ss') AS ordarrtime,to_char(orddeltime, 'yyyy-mm-dd hh24:mi:ss') AS orddeltime,ordstate, memno, empno FROM pro_order where ordgotime LIKE " ;
	@Override
	...
	public Set<ProdItemVO> getProdItemByOrdno(String ordno) {
		Set<ProdItemVO> set = new LinkedHashSet<ProdItemVO>();
		...
		try{
			con = ds.getConnection();
			pstmt = con.prepareStatement(GET_PRODITEM_ByOrdNo_STMT);
			pstmt.setString(1, ordno);
			rs = pstmt.executeQuery();
	...
	public void updateStatus(OrderVO orderVO) {
		...
		try {
			...
			pstmt.setInt(1, orderVO.getOrdstate());
			pstmt.setString(2, orderVO.getOrdno());
			pstmt.executeUpdate();			
		} catch (SQLException se) {
			throw new RuntimeException("A database error occured. "
					+ se.getMessage());
	...
	public Set<OrderVO> getOrdersByGoTime(String date) {
		String sql = GET_ORDERS_GOTIME_TODAY + date;
