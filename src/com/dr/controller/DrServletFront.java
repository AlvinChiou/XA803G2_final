...
public class DrServletFront extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if (contentType != null && contentType.startsWith("multipart/form-data")) {
			multi = new MultipartRequest(req, getServletContext().getRealPath("dr/photo"), 5 * 1024 * 1024, "UTF-8");
...
//			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/front/dr/select_page.jsp");
				...
				String url = "/front/dr/listOneDr.jsp";
...
//			}
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllEmp.jsp的請求
			...
			try {
				...
				String url = "/dr/update_dr_input.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);// 成功轉交
																				// update_dr_input.jsp
				successView.forward(req, res);
		...
		if ("update".equals(action)) { // 來自update_dr_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/dr/update_dr_input.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
		...
		if ("insert".equals(action)) { // 來自addEmp.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/dr/addDr.jsp");
					failureView.forward(req, res);
					return;
				}
