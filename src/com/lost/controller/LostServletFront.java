...
public class LostServletFront extends HttpServlet{
...
//		System.out.println(req.getMethod());
		...
		if(contentType != null && contentType.startsWith("multipart/form-data")){
			multi = new MultipartRequest(req, getServletContext().getRealPath("front/lost/images"), 5*1024*1024, "UTF-8");
			...
			try{
				...
				if(!errorMsgs.isEmpty()){
					RequestDispatcher failureView = req
							.getRequestDispatcher("/front/lost/listAllLost.jsp");
				...
				req.getSession().setAttribute("lostVO", lostVO); // 資料庫取出的lostVO物件,存入req
				String url = "/front/lost/listOneLost.jsp";
		...
		if("listLost_ByMemno".equals(action)){// 來自select_page.jsp的請求
			...
			try{
				...
				String url = "/front/lost/listLost_ByMemno.jsp";
				RequestDispatcher successView = req
		...
		if("listLost_ByState".equals(action)){ // 來自listAllMem.jsp的請求
			...
			try{
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)*************/
				req.setAttribute("list", set); // 資料庫取出的memVO物件,存入req
				String url = "/front/lost/listLost_ByState.jsp";
		...
		if("getOne_For_Update".equals(action)){ // 來自listAllLost.jsp的請求
			...
			try{
				...
				String url = "/front/lost/update_lost_input.jsp";
...
//				try {
					...
					if (!errorMsgs.isEmpty()) {
						...
						RequestDispatcher failureView = req
								.getRequestDispatcher("/front/lost/update_lost_input.jsp");
						failureView.forward(req, res);
						return; //程式中斷
					}
...
//					LostService lostSvc = new LostService();
					...
					String url = "/front/lost/listOneLost.jsp";
					if(requestURL.equals("/front/lost/listAllLost.jsp")){
						req.setAttribute("listOneLost", lostSvc.getOneLost(lostno)); // 資料庫取出的list物件,存入request
						url = "/front/lost/listAllLost.jsp?whichPage=" + whichPage + "&lostno=" + lostno;
		...
		if ("insert".equals(action)) { // 來自addLost.jsp的請求  
				...
				try {
					...
					if (!errorMsgs.isEmpty()) {
						req.setAttribute("lostVO", lostVO); // 含有輸入格式錯誤的lostVO物件,也存入req
						RequestDispatcher failureView = req
								.getRequestDispatcher("/front/lost/addLost.jsp");
						failureView.forward(req, res);
						return;
					}
